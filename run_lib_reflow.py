...
def finetune_reflow(config, workdir):
  ...
  if config.reflow.reflow_type == 'generate_data_from_z0':
      ...
      sys.exit(0)
  elif config.reflow.reflow_type == 'inversion':
      # NOTE: Prepare reflow dataset with ODE
      print('Start generating data with ODE from z1', ', SEED:', config.seed)
      ...
      for i, batch in enumerate(eval_iter):
        # save batch to image
        this_sample_dir = os.path.join(sample_dir, "iter_{}".format(i))
        ...
        with tf.io.gfile.GFile(
            os.path.join(this_sample_dir, "eval.png"), "wb") as fout:
          save_image(image_grid, fout)


        
        eval_batch = scaler(eval_batch)
        z0 = sde.ode(eval_batch, score_model, reverse=True)
        z0_cllt.append(z0.cpu())
        data_cllt.append(eval_batch.cpu())
        z1 = sde.ode(z0, score_model)
        z1_cllt.append(z1.cpu())
        sample1 = inverse_scaler(z1)
        nrow = int(np.sqrt(sample1.shape[0]))
        image_grid = make_grid(sample1, nrow, padding=2)
        sample1 = np.clip(sample1.permute(0, 2, 3, 1).cpu().numpy() * 255, 0, 255).astype(np.uint8)

        # print the difference between sample and sample1
        diff = np.absolute(sample - sample1).mean()
        print('Difference between sample and sample1:', diff)
      ...
      import sys 
      print('Successfully invert eval data with random seed', config.seed, 'Total number of pairs:', config.training.batch_size)
      sys.exit(0)

  elif config.reflow.reflow_type == 'inversion_out_of_distribution':
     # NOTE: Prepare reflow dataset with ODE
      print('Start generating data with ODE from z1', ', SEED:', config.seed)
