...
def psnr_fn(imageA, imageB):
    ...
    # normalize to [0, 1], this is stupid... 
    imageA = (imageA - imageA.min()) / (imageA.max() - imageA.min())
    imageB = (imageB - imageB.min()) / (imageB.max() - imageB.min())
...
# write a center sq bbox that always make the mask at the center of the image
def center_sq_bbox(img, mask_shape, image_size=256, margin=(16, 16)):
    """Generate a random sqaure mask for inpainting
    """
    B, C, H, W = img.shape
    h, w = mask_shape
